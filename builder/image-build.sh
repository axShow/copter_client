...
SOURCE_IMAGE="https://github.com/CopterExpress/clover/releases/download/v0.24/clover_v0.24.img.zip"
...
mkdir -p ${MOUNT_POINT}'/etc/axshow/client/'
for dir in ${REPO_DIR}/*; do
  if [[ $dir != *"images" && $dir != *"imgcache" ]]; then
    cp -r $dir ${MOUNT_POINT}'/etc/axshow/client/'$(basename $dir)
  fi;
done
...
img-chroot ${IMAGE_PATH} copy ${REPO_DIR}'/examples/animations/basic/basic.csv' '/etc/axshow/client/animation.csv'
